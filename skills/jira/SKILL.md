...
acli jira workitem view KEY-123 --web   # Opens the issue in the web browser
```
...
Always create a work item with:

- `project` name provided by user
- `type` of Bug, Task, Story, Doc, or Spike
- `summary` as a short description
- write the complete description in [ADF](./ADF.md) format as a json file

Here is a sample command:

```bash
acli jira workitem create --project PROJ --type Task \
  --summary "Write docs" --description-file ./description.json
```

Additional command options that could be used:
